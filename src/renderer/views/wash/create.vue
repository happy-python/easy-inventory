<template>
  <div class="app-container">
    ...
    >
      ...
      <el-form-item label="洗入产品:" prop="in_product_id_list">
        <el-select
          style="width:40%"
          multiple
          v-model="form.in_product_id_list"
          placeholder="请选择洗入产品（支持多选）"
      ...
      <el-form-item label="洗入数量:" prop="in_product_number_list">
        <el-input
          style="width:40%"
          v-model="form.in_product_number_list"
          placeholder="请依次输入洗入数量（使用、分割，如：100、200）"
          clearable
        ></el-input>
      </el-form-item>

      <el-form-item label="洗出产品:" required>
        <el-select
          style="width:40%"
      ...
      <el-form-item label="洗出数量:" prop="out_product_number">
        <el-input
          style="width:40%"
...
<script>
import { listProduct, createWash } from "@/api/database";
  ...
  methods: {
    ...
    create() {
      if (
        ...
        this.$message.error("洗入数量格式有误，请重新输入");
        return;
      }
      ...
      if (number_list.length != this.form.in_product_id_list.length) {
        this.$message.error(
          `洗入产品个数${this.form.in_product_id_list.length}不等于洗入数量个数${number_list.length}，请重新输入`
        );
        return;
      }
      ...
      for (
        ...
        if (p.length > 0) {
          ...
          if (parseFloat(p[0].quantity) < parseFloat(number_list[index])) {
            this.$message.error(
              `${name}的洗入数量${number_list[index]}超过最大可使用数量${p[0].quantity}，请重新输入`
            );
            return;
          }
        ...
        if (valid) {
          createWash(form)
            .then(() => {
              ...
              setTimeout(() => {
                this.$router.push({ path: "/wash/list" });
              }, 2000);
            })
            .catch((err) => {
